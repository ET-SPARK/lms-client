<template>
  <div class="feature-card-feature-card" v-bind:class="rootClassName">
    ...
    <h2 class="feature-card-text">{{ title }}</h2>
    <span class="feature-card-text1">{{ description }}</span>
  </div>
</template>
...
.feature-card-feature-card {
  @apply w-full flex flex-col p-4 max-w-screen-xl items-start justify-start;
}

.feature-card-icon {
  @apply fill-gray-500 w-8 h-8 mb-4;
}
...
@media (max-width: 479px) {
  .feature-card-text {
    @apply font-normal font-semibold;
  }
}
</style>
